...
def runAI(noChangeData):
  ...
  # Format data
  # -------------------------------------------
  (x_train, y_train, scaler_train) = formatDataset(data, dataForY, batch_size, input_dims, output_dims)
  print(x_train.shape, y_train.shape, len(scaler_train))
  ...
  model.add(LSTM(batch_size, return_sequences=True, activation=tf.keras.activations.tanh))
  model.add(Dropout(0.2))
  model.add(LSTM(batch_size, return_sequences=True, activation=tf.keras.activations.tanh))
  model.add(Dropout(0.2))
  model.add(LSTM(output_dims, activation=tf.keras.activations.tanh))
  ...
  (test_last_x_All, scaler_test) = formatDataPredist(np.copy(data[pos_predist_data:]), batch_size, input_dims)
  print(test_last_x_All.shape)
  test_last_y_All = model.predict(test_last_x_All)
  test_last_y_All = descaler_block(test_last_y_All, scaler_test, plus=True).T
  print(test_last_y_All.shape, len(scaler_test))

  (pos_l, pos_r) = find_best_pos(test_last_y_All[NumP:, 0]);
  print(pos_l, pos_r)
  BackPrize = test_last_y_All[pos_l, 0]
  test_last_y = test_last_y_All[pos_r, 0]
  ...
  (test_x, scaler_all) = formatDataPredist(CopyData, batch_size, input_dims)
  test_y = model.predict(test_x)
  test_y = descaler_block(test_y, scaler_all)
  test_y = test_y.T
  print("Draw: ", test_y.shape)
  ...
  DrawPlot(orgData_close, 0, 'Train 0', date = [], step = 1, noinv = True)
  
  for i in range(0, output_dims):
    DrawPlot(test_y[i], batch_size + i, 'P' + str(i), date = [], step = 1, noinv = True)
  
  valSpitPos = len(CopyData)
  minline = np.min(orgData_close)
  maxline = np.max(orgData_close)
...
def scaler_block(xValue):
  global fea_range
  xValue_0_3 = np.copy(xValue[0:, 0:4])
  scaler = MinMaxScaler(feature_range=fea_range)
  ...
  scaler_volume = MinMaxScaler(feature_range=fea_range)
  ...
  scaler_MACD = MinMaxScaler(feature_range=fea_range)
  ...
  xValue_6_9 = (xValue_6_9-50)/25
  xValue[0:, 6:10] = xValue_6_9
  
  for i in range(10, numInput):
    xValue_i = np.copy(xValue[0:, i])
    scaler_i = MinMaxScaler(feature_range=fea_range)
    xValue_i = scaler_i.fit_transform(xValue_i.reshape((-1, 1)))
    xValue_i = xValue_i.reshape((-1, 1))
    xValue[0:, i] = xValue_i.ravel()

  return (xValue, scaler);
def descaler_block(xValue, scaler, plus=False):
  global fea_range
  for i in range(0, len(xValue)):
    xValue[i] = scaler[i].inverse_transform(xValue[i].reshape((-1, 1))).reshape((-1))
    xValue[i] = np.float64(xValue[i])
...
def formatDataset(data, dataForY, scaler_x, num_in, num_out):
  ...
  for i in range(0, numData):
    data_scalex = np.copy(data[i:i+scaler_x+num_out])
    (data_scalex, scaler[i]) = scaler_block(data_scalex)
    x[i] = np.copy(data_scalex[0:scaler_x]);
    y[i] = np.copy(data_scalex[scaler_x: scaler_x+num_out, 3])
...
def formatDataPredist(data, scaler_x, num_in):
  ...
  scaler = [None]*numData
  for i in range(0, numData):
    data_scalex = np.copy(data[i:i+scaler_x])
    (data_scalex, scaler[i]) = scaler_block(data_scalex)
    x[i] = np.copy(data_scalex[0:scaler_x]);
  return (x, scaler)
...
def cal_bollinger_bands(df, n=20):
  df = pd.DataFrame(df)
  """
  ...
  """
  MA = pd.Series(df[_DP['Close']].rolling(n, min_periods=n).mean())
  MSD = pd.Series(df[_DP['Close']].rolling(n, min_periods=n).std())
  b1 = 4 * MSD / MA
  B1 = pd.Series(b1, name='BollingerB_' + str(n))
  df[_DP['BBU'+str(n)]] = B1

  b2 = (df[_DP['Close']] - MA + 2 * MSD) / (4 * MSD)
...
def cal_MA(df, n):
  ...
  :return: pandas.DataFrame
  """
  MA = pd.Series(df[_DP['Close']].rolling(n, min_periods=n).mean(), name='MA_' + str(n))
  df[_DP['MA'+str(n)]] = MA
  return df
...
def updateGraph(i):
    ...
    SaveData = cal_MA(SaveData, 10).to_numpy(dtype=np.float64)
    SaveData = cal_MA(SaveData, 20).to_numpy(dtype=np.float64)
    SaveData = cal_MA(SaveData, 50).to_numpy(dtype=np.float64)
    SaveData = cal_MA(SaveData, 100).to_numpy(dtype=np.float64)
    SaveData = cal_bollinger_bands(SaveData).to_numpy(dtype=np.float64)
...
print("1: EURUSDm | 2: USOILm | 3: XAUUSDm | 4: BTCUSDm | 5: ETHUSDm | 6: CUS => ")
idStock = input()
...
if int(idStock) == 5:
    ...
    min_delta_cmp = 80
if int(idStock) == 6:
    input_stockName = input()
    nameFile = "data_"+str(input_stockName)+"m"
    type_tick = str(input_stockName)+"m"
    min_delta_cmp = 0.1000
...
# Zoom
# -------------------------------------------
cap_range_up = batch_size + 50
cap_range_up_sub = batch_size + 50
