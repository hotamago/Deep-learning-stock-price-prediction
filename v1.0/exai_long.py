...
def runAI(noChangeData):
  ...
  plt.plot([valSpitPos + pos_l, valSpitPos + pos_l], [minline, maxline], 'r-', lw=1)
  plt.plot([valSpitPos + pos_r, valSpitPos + pos_r], [minline, maxline], 'g-', lw=1)
...
def updateGraph(i):
    ...
    assert not np.any(np.isnan(SaveData[len(SaveData)-NumDataPerTest:, 7]))
    # SaveData = enmathPre(SaveData)
...
#anima = animation.FuncAnimation(plt.gcf(), updateGraph, interval=60000)
